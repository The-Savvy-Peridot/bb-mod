...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'familial-bonds':
				dialogue = CoolUtil.coolTextFile(Paths.txt('familial-bonds/familial-bondsDialogue'));
			case 'always-here':
				dialogue = CoolUtil.coolTextFile(Paths.txt('always-here/always-hereDialogue'));
			case 'memories':
				dialogue = CoolUtil.coolTextFile(Paths.txt('memories/memoriesDialogue'));	
			case 'heartache':
				dialogue = CoolUtil.coolTextFile(Paths.txt('heartache/heartacheDialogue'));
			case 'heartacheend':
				dialogue = CoolUtil.coolTextFile(Paths.txt('heartacheend/heartacheendDialogue'));
		}
		...
		switch (SONG.song.toLowerCase())
		{
		          ...
		          case 'thorns':
		          {
				   ...
				   case 'familial-bonds' | 'memories' | 'heartache' | 'heartacheend': 
            	  		{
		            	  curStage = 'prologue';

		            	  var bg:FlxSprite = new FlxSprite(-100).loadGraphic(Paths.image('prologue/rooftopsky'));
		            	  bg.scrollFactor.set(0.1, 0.1);
		            	  add(bg);
					...
					case 'always-here':
						  ...
						  var street:FlxSprite = new FlxSprite(-170, streetBehind.y).loadGraphic(Paths.image('prologue/rooftop'));
				                add(street);
	...
	function coolEnd(?dialogueBox:DialogueBox):Void
		{
			...
			camHUD.visible = false;
			aft.setGraphicSize(Std.int(aft.width * 1));
			aft.scrollFactor.set();
			var black:FlxSprite = new FlxSprite(-100, -100).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
			black.scrollFactor.set();
			add(aft);
			add(black);
	...
	override public function update(elapsed:Float)
	{
		...
		if (SONG.song.toLowerCase() == 'familial-bonds')
			{
				switch (curBeat)
				{
					case -4:
		...
		if (SONG.song.toLowerCase() == 'always-here')
			{
				switch (curBeat)
				{
					case -4:
	...
	function waveBye():Void
	{
		...
		#end
		#if desktop
		var bye:FlxSprite = new FlxSprite(173, 189).loadGraphic(Paths.image('cutscenes/aftermath3'));
		#end
